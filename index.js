...
function getUsersFromDatabase(array) {
  ...
  //якого це об'єкти які міститять
  // id який дорівнює id користувача,firstName який дорівнює firstName користувача в верхньому
  // регістрі та years який дорівнює age користувача
...
function customAt(arr, index) {
  ...
  // перевіремо якого типу елемент ми отримали,якщо рядок виведом в консоль -Елемент
  //є рядком,числом - Елемент є числом, об`єктом -Елемент є обєктом
...
function customIncludes(arr, element) {
  ...
  const elemIncludes = arr.includes(element);
  const event = arr.filter((item) => item === element).length;
  // Перевіряємо, чи вхідний параметр є масивом
  ...
  console.log(`Кількість зустрічей елемента ${element} в масиві: ${event}`);
